...
export const RescheduleDialog = ({
  ...
  const minDateYear = minDate
    ? minDate
    : new Date(new Date().setFullYear(new Date().getFullYear() + 1));
  console.log("Min Date Year:", minDateYear);
  ...
  const maxBookingDate = (() => {
    const futureDates = Object.keys(availability)
      .map((d) => new Date(d))
      .filter((d) =>
        availability[format(d, "yyyy-MM-dd")].some(
          (a) => a.maxCapacity !== null
        )
      );
  ...
  const isDateDisabled = (date: Date) => {
    ...
    return (
      isBefore(date, minBookingDate) || // before min
      isWarehouseClosed(date) || // warehouse closed
      isAfter(date, maxBookingDate) || // after max allowed
      (apiEntry ? apiEntry.some((a) => !a.available) : isLoadingAvailability) // unavailable
    );
  };
  ...
  const handleReschedule = async () => {
    ...
    try {
      ...
      setSuccessMessage(
        `Booking rescheduled to ${format(selectedDate, "MMM d, yyyy")}`
      );
      setTimeout(() => {
        setOpen(false);
  ...
  const isWarehouseClosed = (date: Date) => {
    const dateStr = format(date, "yyyy-MM-dd");
    ...
    return entries.every((a) => a.maxCapacity === 0) || (isWeekend && isFuture);
  };
              ...
              {isLoadingAvailability ? (
                ...
                <Calendar
                  ...
                  onSelect={(date) => {
                    if (!date) {
                      setError(
                        "Date has not been scheduled and therefore not available"
                      );
                      return;
                    }
                    const dateStr = format(date, "yyyy-MM-dd");
                  ...
                  modifiers={{
                    unavailable: Object.keys(availability)
                      .map((dateStr) => {
                        ...
                        return entries.some(
                          (a) =>
                            !a.available && a.maxCapacity !== null && !isWeekend
                        )
                          ? new Date(dateStr)
                          : null;
                      })
                    ...
                    available: Object.keys(availability)
                      .map((dateStr) => {
                        ...
                        const isWeekend =
                          date.getDay() === 0 || date.getDay() === 6;
                        const isFuture = date > new Date();
                        return entries.some((a) => a.available && !isWeekend)
                          ? new Date(dateStr)
                          : null;
                      })
                      .filter(Boolean) as Date[],
                    warehouseClosed: Object.keys(availability)
                      .map((dateStr) => {
                        const date = new Date(dateStr);
                        const entries = availability[dateStr];
                        ...
                        if (
                          entries.every((a) => a.maxCapacity === 0) ||
                          (isWeekend && isFuture)
                        ) {
                          return date;
                        }
                  ...
                  modifiersClassNames={{
                    unavailable:
                      "bg-red-100 text-red-600 line-through cursor-not-allowed rounded-lg hover:bg-red-200 relative",
                    available:
                      "bg-gradient-to-b from-green-100 to-green-200 border border-green-300 text-black rounded-lg",
                    disabled:
                      "bg-gray-100 text-gray-400 cursor-not-allowed rounded-lg",
                    warehouseClosed:
                      "bg-amber-50 text-amber-700 cursor-not-allowed rounded-lg border border-amber-200 hover:bg-amber-100 relative after:content-['🔒'] after:absolute after:inset-0 after:flex after:items-center after:justify-center after:text-3xl after:opacity-70",
                    missing:
                      "bg-gray-50 text-gray-300 cursor-not-allowed rounded-lg",
                  }}
                  // classNameDayContent="relative"
                  className="w-full [&_[role=gridcell]]:m-[1px]"
            ...
            <div className="rounded-lg border border-gray-200 bg-gray-50 p-3">
              ...
              <div className="space-y-2 text-xs">
                <div className="flex items-center gap-2">
                  <div className="w-4 h-4 rounded bg-gradient-to-b from-green-400 to-green-500" />
                ...
                <div className="flex items-center gap-2">
                  <div className="w-4 h-4 rounded bg-amber-100 border border-amber-200 flex items-center justify-center text-xs">
                    🔒
                  </div>
                  <span className="text-gray-600">Warehouse Closed</span>
                </div>
                <div className="flex items-center gap-2">
