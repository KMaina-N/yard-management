...
import { handlePrintPDF } from "@/lib/pdfGenerator";
...
export default function BookingConfirmation({
  ...
  const handlePrint = async() => {
    return await handlePrintPDF(bookingData, selectedDate, selectedTime, selectedBay, productTypes, bookingId);
  }
      ...
      <Card className="border border-neutral-200 shadow-sm">
        <CardHeader className="text-center space-y-1 pb-2">
          ...
          <p className="text-xs text-neutral-600 max-w-sm mx-auto">
            Your booking has been received and will be confirmed shortly. Please
        ...
        <CardContent className="space-y-6 p-4">
          ...
          <div className="space-y-1 border border-neutral-200 rounded-lg p-2 bg-neutral-50 text-sm">
            <SummaryRow
              label="Items"
              value={bookingData.goods.reduce((t, i) => t + i.quantities, 0)}
            />
            {bookingData.goods.reduce((t, i) => t + i.numberOfPallets, 0) >
              0 && (
              <SummaryRow
                label="Pallets"
                value={bookingData.goods.reduce(
                  (t, i) => t + i.numberOfPallets,
                  0
                )}
              />
            )}
