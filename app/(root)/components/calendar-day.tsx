...
const CalendarDay: React.FC<CalendarDayProps> = ({
  ...
  const getDayColor = () => {
    ...
    if (isFull) return "bg-red-100 line-through text-red-800 border-red-200";

    const minRatio = Math.min(
      ...info.products.map((p) => p.remaining / p.maxCapacity)
    );
    if (minRatio > 0.50) return "bg-green-100 text-green-800 border-green-300";
    // if (minRatio < 0.33)
    //   return "bg-yellow-100 text-yellow-800 border-yellow-300";
    return "bg-orange-100 text-orange-800 border-orange-300";
  };

  console.log("Info for date", format(date, "PPP"), info);
  ...
  return (
    ...
    // </Tooltip>
    <div
      onClick={() => !isDisabled || !isFull && onSelect(date)}
      className={cn(
        "flex flex-col items-center p-2 h-20 rounded-xl transition relative overflow-hidden",
        getDayColor(),
        isSelected ? "ring-1 ring-primary ring-offset-2 shadow-2xl" : "",
