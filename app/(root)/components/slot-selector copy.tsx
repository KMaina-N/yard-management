  ...
  const fetchAvailability = async () => {
    ...
    try {
      ...
      const availability: Record<
        string,
        { full: boolean; messages: string[]; products: ProductAvailability[] }
      > = {};
      const nextDays: { date: string; products: ProductAvailability[] }[] = [];

      for (const [dateStr, products] of Object.entries(data.availability)) {
        const prods = products as ProductAvailability[];

        // A day is full if all products are unavailable
        // const full = prods.every(
        //   (p) => !p.available || (p.remaining <= 0 && p.remaining !== null)
        // );
        let full = prods.every(
          (p) => !p.available && p.remaining !== null
        );

        
        // Messages for unavailable products
        const messages = prods
          .filter((p) => !p.available || p.remaining <= 0)
          .map((p) => `${p.message} (${p.remaining}/${p.maxCapacity})`);

        availability[dateStr] = { full, messages, products: prods };

        // Add to nextDays if the day is not full and is today or in the future
        const dayDate = new Date(dateStr);
        const today = new Date();
        today.setHours(0, 0, 0, 0);
        if (!full && dayDate >= today) {
          nextDays.push({ date: dateStr, products: prods });
        }
      }

      // Sort next available days and take first 10
