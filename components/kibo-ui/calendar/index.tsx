...
export const CalendarBody = ({ features, children }: CalendarBodyProps) => {
  ...
  for (let day = 1; day <= daysInMonth; day++) {
    ...
    days.push(
      <ScrollArea
        className="relative flex h-full w-full flex-col gap-1 p-1 text-muted-foreground text-xs overflow-auto"
        key={day}
      >
        {day}
        <div>
        ...
        {featuresForDay.length > 3 && (
          <span className="block text-muted-foreground text-xs cursor-pointer" onClick={() => handleExpand(day)}>
            {expandedDays.includes(day) && (
              <div className="">
                {featuresForDay.slice(3).map((feature) => children({ feature }))}
              </div>
            )}
            {expandedDays.includes(day) ? "Show less" : `+${featuresForDay.length - 3} more`}
          </span>
        )}
      </ScrollArea>
    );
  ...
  return (
    <div className="grid flex-grow grid-cols-7 ">
      {days.map((day, index) => (
        <div
          className={cn(
            "relative h-32 overflow-hidden border-t border-r",
...
export const Modal = ({feature}: any) => {
  console.log(feature);
